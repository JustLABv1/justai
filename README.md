...
- Chat, voice, and transcription stream server events over resumable SSE. Voice and transcription send sequenced controls and numbered, retry-safe audio batches through HTTP uploads protected by separate short-lived upload tokens. Current clients send `X-Audio-Batch-Sequence`; the server accepts each numbered batch once per stream and bounds queued audio for backpressure. Unnumbered uploads from older clients remain accepted during rollout.
