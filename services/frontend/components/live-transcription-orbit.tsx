...
export function LiveTranscriptionOrbit({
  ...
  const dominantSourceId = useMemo(() => {
    ...
    return activeSources.reduce<string | null>((dominant, source) => {
  ...
  const mappedSpeakers = useMemo(() => {
    if (snapshot.speakers.length > 0) {
      return snapshot.speakers.map((speaker, index) => {
        ...
        const name =
          speaker.displayName || speaker.label || `Speaker ${index + 1}`
        return {
          id: speaker.id,
          name,
          ...
          signal,
          accent:
            speaker.color || speakerAccents[index % speakerAccents.length],
          status:
            source?.id === dominantSourceId &&
    ...
    return snapshot.sources.map((source, index) => {
      ...
      return {
        ...
        status:
          source.id === dominantSourceId && snapshot.session.status === "live"
            ? "speaking"
            : source.status === "connected"
              ? "listening"
              : "quiet",
  ...
  const activeSpeaker = dominantSourceId
    ? mappedSpeakers.find((speaker) => speaker.sourceId === dominantSourceId) ||
      null
    : null
  const selectedSpeaker =
    mappedSpeakers.find((speaker) => speaker.id === selectedSpeakerId) || null
  const selectedIsRealSpeaker = Boolean(
    selectedSpeaker &&
    snapshot.speakers.some((speaker) => speaker.id === selectedSpeaker.id)
  )
    ...
    if (partial) {
      lines.push({
        ...
        speakerId:
          partialSpeakerId ||
          (partialSourceId
            ? `source:${partialSourceId}`
            : activeSpeaker?.id || "unassigned"),
        text: partial,
        provisional: true,
      })
    }
  ...
  const selectSpeaker = (speaker: RoomSpeaker) => {
    ...
    if (snapshot.speakers.some((item) => item.id === speaker.id))
      setDetailOpen(true)
  }
    ...
    <div className={orbitStyles.productionVariant}>
      ...
      <div className={orbitStyles.productionContent}>
        ...
        <div className={orbitStyles.orbitWorkspace}>
          <section
            aria-label="Speaker orbit"
            className={orbitStyles.orbitCanvas}
          >
            <div className={orbitStyles.orbitHint}>
              <Badge variant="secondary">
                <CircleDot data-icon="inline-start" /> Room view
              </Badge>
              <span className={orbitStyles.orbitHintText}>
                {snapshot.speakers.length > 0
                  ? "Anonymous speaker labels are editable"
                  : "Sources are ready; labels arrive after diarization"}
              </span>
            </div>
            ...
            <div className="absolute bottom-5 left-1/2 z-[2] -translate-x-1/2 text-center">
              <p className="text-xs font-medium">
                {activeSpeaker
                  ? `${activeSpeaker.source} is dominant`
                  : "Waiting for audio"}
              </p>
          ...
          <aside className={orbitStyles.orbitInspector}>
            <Card
              ...
              style={{ "--speaker-accent": selectedAccent } as CSSProperties}
            >
              <CardHeader className="gap-2 px-4 py-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge
                    variant={
                      selectedSpeaker?.status === "speaking"
                        ? "default"
                        : "secondary"
                    }
                  >
              ...
              <CardContent className="flex flex-col gap-3 px-4 pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <Avatar size="lg">
                      <AvatarFallback
                        style={
                          {
                            background: selectedSpeaker.accent,
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate text-[11px] text-muted-foreground">
                        {selectedSpeaker.source} · {selectedSpeaker.device}
                      </p>
                    </div>
                  </div>
                ) : (
                ...
                <div className="flex items-center justify-between gap-2 rounded-xl bg-muted/55 px-3 py-2">
                  ...
                  <span className="flex items-center gap-2 text-xs font-medium">
                    <SignalBars
                      accent={selectedSpeaker?.accent}
                      value={
                        selectedSpeaker?.signal ??
                        Math.round(clamp(focusedSource?.signalLevel ?? 0) * 100)
                      }
                    />{" "}
                    {selectedSpeaker?.signal ??
                      Math.round(clamp(focusedSource?.signalLevel ?? 0) * 100)}
                ...
                {selectedIsRealSpeaker ? (
                  <Button
                    onClick={() => setDetailOpen(true)}
                    size="sm"
                    variant="outline"
                  >
                    <Pencil data-icon="inline-start" /> Edit speaker details
                  </Button>
                ) : null}
              </CardContent>
            </Card>
            ...
            <ProductionSourceSummary
              activeSourceId={focusedSourceId || dominantSourceId}
              onSourceChange={setFocusedSourceId}
                ...
                <CardContent className="flex flex-col gap-2 px-4 pb-4">
                  {pendingRequests.map((request) => (
                    <div className="flex items-center gap-2" key={request.id}>
                      ...
                      <Button
                        aria-label={`Approve ${request.sourceName}`}
                        onClick={() =>
                          void onSetJoinRequest(request, "approve")
                        }
                        size="icon-sm"
                        variant="outline"
                      >
                      ...
                      <Button
                        aria-label={`Deny ${request.sourceName}`}
                        onClick={() => void onSetJoinRequest(request, "deny")}
                        size="icon-sm"
                        variant="ghost"
                      >
                        <X data-icon="inline-start" />
            ...
            <Card className="shadow-none">
              ...
              <CardContent className="grid grid-cols-2 gap-2 px-4 pb-4">
                <HealthMetric
                  label="Sources"
                  value={String(snapshot.sources.length)}
                />
                <HealthMetric
                  label="Confidence"
    ...
    <Card className="shadow-none">
      ...
      <CardContent className="flex flex-col gap-1 px-3 pb-3">
        {sources.length === 0 ? (
          ...
          sources.map((source) => {
            const sourceSpeaker = speakers.find(
              (speaker) => speaker.sourceId === source.id
            )
            ...
            return (
              <Button
                aria-pressed={active}
                className={cn(
                  "h-auto justify-start gap-2 rounded-xl px-2 py-2 text-left",
                  active && "bg-muted"
                )}
                key={source.id}
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block truncate text-[10px] text-muted-foreground">
                    {source.deviceLabel || source.kind} · {source.status}
                  </span>
                </span>
...
function RecordingCard({
  ...
  return (
    <Card>
      <CardHeader className="gap-1 px-4 py-4">
        <CardTitle className="text-sm">Audio tracks</CardTitle>
        <CardDescription className="text-[11px]">
          Source recordings remain available until retention expires.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 px-4 pb-4">
        {recordings.map((recording) => (
          <div
            ...
            key={`${recording.id}-${recording.completedAt ?? "active"}`}
          >
            <div className="flex items-center justify-between gap-2 text-[10px] text-muted-foreground">
              <span className="truncate">
                {sources.find((source) => source.id === recording.sourceId)
                  ?.name || "Source"}
              </span>
              <span className="shrink-0">
                {recording.expiresAt
                  ? `Until ${new Date(recording.expiresAt).toLocaleDateString()}`
                  : ""}
              </span>
            </div>
...
function RecordingAudio({ recording }: { recording: TranscriptionRecording }) {
  ...
  useEffect(() => {
    ...
    let objectURL = ""
    void api
      .getBlob(`/api/v1/transcription/recordings/${recording.id}`)
      .then((blob) => {
        if (cancelled) return
  ...
  }, [recording.id, recording.completedAt])

  if (error) return <p className="text-xs text-destructive">{error}</p>
  if (!sourceURL)
    return <p className="text-xs text-muted-foreground">Loading audio…</p>
  return <audio controls preload="none" src={sourceURL} />
}
