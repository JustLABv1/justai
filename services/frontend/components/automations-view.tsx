...
type Props = {
  ...
  servers: MCPServer[]
  onChange: (items: Automation[]) => void
}
type Form = {
  name: string
  prompt: string
...
export function AutomationsView({
  ...
  const availableServers = useMemo(
    () =>
      servers.filter((server) => server.enabled && server.credentialConfigured),
    [servers]
  )
  ...
  async function save() {
    ...
    setError("")
    try {
      const result = await api.post<{ automation: Automation }>(
        "/api/v1/automations",
        form
      )
  ...
  function toggleServer(id: string) {
    setForm((current) => ({
      ...current,
      mcpServerIds: current.mcpServerIds.includes(id)
        ? current.mcpServerIds.filter((value) => value !== id)
        : [...current.mcpServerIds, id],
    }))
  }
  ...
  return (
    <Page>
      <PageHeader>
        <PageHeading>
          <PageEyebrow>Automations</PageEyebrow>
          <PageTitle>Give assistants recurring work.</PageTitle>
          <PageDescription>
            Schedule a focused task, choose the assistant and restrict exactly
            which integrations it may use.
          </PageDescription>
        </PageHeading>
        <PageActions>
          <Button
      ...
      <Alert className="border-primary/20 bg-primary/[0.03]">
        ...
        <AlertDescription>
          Integration writes always require review. Only explicitly trusted
      ...
      {automations.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex min-h-64 flex-col items-center justify-center text-center">
            ...
            <h2 className="mt-4 font-medium">Nothing scheduled yet</h2>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              Create a recurring research brief, issue triage or follow-up –
              with a small, intentional MCP scope.
            </p>
            <Button
              className="mt-5"
              variant="outline"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {automations.map((item) => {
            const assistant = assistants.find(
              (entry) => entry.id === item.assistantId
            )
            const selectedServers = servers.filter((server) =>
              item.mcpServerIds.includes(server.id)
            )
            const itemRuns = runs[item.id]
            return (
