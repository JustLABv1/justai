...
export function upsertTranscriptionSource(
  ...
  const next: TranscriptionSource = {
    ...
    protocol: source.protocol ?? existing?.protocol,
    platform: source.platform ?? existing?.platform,
    transportStatus: source.transportStatus ?? existing?.transportStatus,
    reconnectCount: source.reconnectCount ?? existing?.reconnectCount,
    lastError: source.lastError ?? existing?.lastError,
    status: nextStatus,
